...
test("セッション閾値は環境変数で変更できる", () => {
  assert.deepEqual(
    loadSessionPolicy({
      CLAUDE_SESSION_MAX_REQUESTS: "10",
      CLAUDE_SESSION_MAX_CONTEXT_TOKENS: "50000",
      CLAUDE_HANDOFF_MAX_CHARS: "2000",
    }),
...
test("0・小数・文字列の閾値を拒否する", () => {
  assert.throws(
    () => loadSessionPolicy({ CLAUDE_SESSION_MAX_REQUESTS: "0" }),
    SessionPolicyError,
  );
  assert.throws(
    () => loadSessionPolicy({ CLAUDE_SESSION_MAX_CONTEXT_TOKENS: "1.5" }),
    SessionPolicyError,
  );
  assert.throws(
    () => loadSessionPolicy({ CLAUDE_HANDOFF_MAX_CHARS: "長い" }),
    SessionPolicyError,
  );
});
