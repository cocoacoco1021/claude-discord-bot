...
export function loadSessionPolicy(environment = process.env) {
  return {
    maxRequests: readPositiveInteger(
      environment,
      "CLAUDE_SESSION_MAX_REQUESTS",
      DEFAULT_MAX_REQUESTS,
    ),
    maxContextTokens: readPositiveInteger(
      environment,
      "CLAUDE_SESSION_MAX_CONTEXT_TOKENS",
      DEFAULT_MAX_CONTEXT_TOKENS,
    ),
    maxHandoffChars: readPositiveInteger(
      environment,
      "CLAUDE_HANDOFF_MAX_CHARS",
 ...
 * 入力: 最大文字数。出力: Claudeへ渡す要約指示。
 */
export function buildHandoffSummaryPrompt(maxHandoffChars) {
  return (
    "新しいClaudeセッションへ引き継ぐため、ここまでの会話を要約してください。" +
 ...
 * 入力: 持ち主の依頼と引き継ぎ要約。出力: Claudeへ渡す本文。
 */
