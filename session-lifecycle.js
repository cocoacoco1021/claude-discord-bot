...
/**
 * 役割: Discordの依頼とClaudeセッションの更新・引き継ぎを管理する。
 * 入力: 初期状態、閾値、永続化・Claude呼出・再試行判定・要約保管の各関数。
 * 出力: セッション管理サービス。
 */
export class SessionLifecycle {
  constructor({
    ...
    this.invokeClaude = invokeClaude;
  ...
  async runOwnerPrompt(prompt, imagePaths = [], { onRotationStart } = {}) {
    ...
    const claudeResult = await this.invokeWithInvalidSessionRecovery(
      promptWithHandoff,
      imagePaths,
    );
    const nextSessionId = claudeResult.sessionId ?? this.sessionState.sessionId;

    this.commitState({
      sessionId: nextSessionId,
      requestCount: this.sessionState.requestCount + 1,
      contextTokens: normalizeContextTokens(claudeResult.contextTokens),
      // 新しい会話IDへ要約を渡せた時点で、保存側からは取り除く。
      handoffSummary: nextSessionId ? null : this.sessionState.handoffSummary,
      rotationPending: false,
    });

    return { text: claudeResult.text, rotation };
  }
  ...
  async rotateWithHandoff() {
    ...
    let claudeResult;
    try {
      claudeResult = await this.invokeClaude({
  ...
  async invokeWithInvalidSessionRecovery(prompt, imagePaths) {
    try {
      ...
      return this.invokeClaude({ prompt, sessionId: null, imagePaths });
    }
  }
